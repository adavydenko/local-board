...
<script>let token='';login.onclick=()=>{token=document.querySelector('#token').value;load()};
async function load(){let r=await fetch('/api/issues');let xs=await r.json();columns.textContent='Issues: '+xs.map(x=>x.title).join(', ')}
create.onclick=async()=>{let ps=await(await fetch('/api/projects')).json();await fetch('/api/issues',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+token},body:JSON.stringify({project_id:ps[0].id,title:title.value})});load()};load()</script>"""

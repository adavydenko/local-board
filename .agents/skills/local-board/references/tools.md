...
- `add_git_link(issue, ref?, refs?, kind?, url?, return_full_issue?)` rev= — pass `ref` or `refs`; kind: commit (default) | pr | mr; returns the created link(s) with their ids. Branches are not linked: the issue id in the branch name is the association, inside git itself

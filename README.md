...
The suite is split into unit, integration, end-to-end, and concurrency levels.
Run it with branch coverage and a missing-lines report:
...
The 75% branch-coverage baseline is based on the complete suite, including
subprocess-driven CLI scenarios that intentionally execute outside the parent
coverage process. Missing lines are printed locally and the CI run publishes an
XML report for inspection.

Browser dependencies are deliberately isolated from the regular test install:

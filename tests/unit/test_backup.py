...
class BackupTest(unittest.TestCase):
    ...
    def test_round_trip_backup_with_manifest(self):
        self.board.create_project(self.actor["id"], "SAFE", "Safe")
        ...
        self.board.create_project(self.actor["id"], "TEMP", "Temporary")
        restore_backup(self.board, backup)
        self.assertEqual([project["key"] for project in self.board.list_projects()], ["SAFE"])
    ...
    def test_rejects_non_database_and_tampered_manifest(self):
        ...
        value = json.loads(manifest.read_text()); value["sha256"] = "0" * 64

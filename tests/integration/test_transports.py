...
class HttpIntegrationTest(unittest.TestCase):
    ...
    def test_http_requires_a_valid_token(self):
        with urlopen(self.url + "/", timeout=3) as response:
            ...
            self.assertIn("Local Board", response.read().decode())
        with self.assertRaises(HTTPError) as caught:
            self.request("/api/dashboard", token=False)
        self.assertEqual(caught.exception.code, 401)
        request = Request(self.url + "/api/dashboard", headers={"Authorization": "Bearer invalid"})
        with self.assertRaises(HTTPError) as invalid:
            urlopen(request, timeout=3)
        self.assertEqual(invalid.exception.code, 401)
        self.assertEqual(json.load(invalid.exception)["error"]["code"], "unauthorized")

    def test_http_reports_bad_requests_and_missing_routes(self):
        headers = {"Authorization": f"Bearer {self.actor['token']}", "Content-Type": "application/json"}
        malformed = Request(self.url + "/api/issues", data=b"{", headers=headers)
        ...
        self.assertEqual(json.load(bad_request.exception)["error"]["code"], "invalid_request")
        missing = Request(self.url + "/api/issues/HTTP-999", headers=headers)
        with self.assertRaises(HTTPError) as not_found:
            urlopen(missing, timeout=3)
        self.assertEqual(not_found.exception.code, 404)
        self.assertEqual(json.load(not_found.exception)["error"]["code"], "not_found")
